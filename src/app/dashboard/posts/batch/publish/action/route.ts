...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("batch.publish.post.resp:", resp);
    return NextResponse.json({
      ok: true,
      message: `${resp.docs.length} Post${resp.docs.length > 1 ? "s" : ""} published successfully`,
      data: { ids: resp.docs.map(({ id }) => id) },
    });
  } catch (e: any) {
    console.error("batch.publish.post.err:", e);
    return NextResponse.json(
