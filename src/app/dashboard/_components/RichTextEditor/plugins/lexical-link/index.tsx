...
import { LinkFields, LinkPayload, SerializedLinkNode } from "./types";
...
export class LinkNode extends ElementNode {
  ...
  constructor({
    ...
    super(key);
    console.log("LinkNode.constructor", id, fields, key);
    this.__id = id;
    this.__fields = fields;
  }

  static override clone(node: LinkNode): LinkNode {
    console.log("LinkNode.clone", node);
    return new LinkNode({
      id: node.__id,
  ...
  override updateDOM(
    ...
    if (
      url != null &&
      url !== prevNode.__fields?.url &&
      this.__fields?.linkType === "custom"
    ) {
      anchor.href = url;
    }
    if (
      this.__fields?.linkType === "internal" &&
      prevNode.__fields?.linkType === "custom"
    ) {
...
export function $toggleLink(
  payload: ({ fields: LinkFields } & LinkPayload) | null,
): void {
  const selection = $getSelection();

  if (
    !$isRangeSelection(selection) &&
    (payload === null || !payload.selectedNodes?.length)
  ) {
    return;
  }
...
function manageRel(
  ...
  if (action === "add") {
    ...
    result =
      mutableInput.length === 0 ? `${value}` : `${mutableInput} ${value}`;
  } else {
    const re = new RegExp(value, "g");
