...
import { transactions } from "@/app/dashboard/_mocks/statements";
...
const Transactions = () => {
  ...
  return (
    <div className="card">
      ...
      {search !== "" ? (
        <NoFound title="No transactions found" collectionName="" />
      ) : (
          ...
          >
            {transactions.map((transaction) => (
              <TableRow key={transaction.id}>
                ...
                <td className="max-md:hidden">{transaction.date}</td>
                <td className="max-lg:hidden">{transaction.orderId}</td>
                ...
                <td className="text-right text-sub-title-2 max-md:!pl-0">
                  <NumericFormat
                    className={transaction.amount < 0 ? "text-primary-03" : ""}
